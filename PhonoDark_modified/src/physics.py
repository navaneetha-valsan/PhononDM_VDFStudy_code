...
def calc_diff_rates_SI(mass, threshold, q_XYZ_list, G_XYZ_list, jacob_list, physics_parameters,
                    vE_vec, numerics_parameters, phonopy_params, ph_omega, ph_eigenvectors,
...
### SHM, TSA, EMP ###
                if const.VDF == 'SHM':
                    g0_val = vel_g_function_integrals.g0_func_SHM(q_vec, energy_diff, mass, 
                                            vE_vec, v_minus_val)
                elif const.VDF == 'TSA':
                    g0_val = vel_g_function_integrals.g0_func_TSA(q_vec, energy_diff, mass, 
                                            vE_vec, v_minus_val)
                else:
                    g0_val = vel_g_function_integrals.g0_func_EMP(q_vec, energy_diff, mass, 
                                            vE_vec, v_minus_val)
    ...
    total_rate = sum(diff_rate)   ### original

    return [diff_rate, binned_rate, total_rate]  ### original
