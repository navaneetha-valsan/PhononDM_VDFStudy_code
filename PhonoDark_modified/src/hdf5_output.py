...
def hdf5_write_output(out_file, numerics_parameters, physics_parameters, dm_properties_dict, c_dict,
    ...
    out_f.create_group('astrophysical')

    V0=str(const.V0/const.kmet_per_sec_to_none)
    VE=str(const.VE/const.kmet_per_sec_to_none)
    VESC=str(const.VESC/const.kmet_per_sec_to_none)
    RHO_DM=str(const.RHO_DM/(const.GeV_To_eV*const.invcmet_To_eV**3))

    out_f.create_dataset('astrophysical/VDF', data=(const.VDF)+' Model')
    out_f.create_dataset('astrophysical/V0', data=V0+' Km/s')
    out_f.create_dataset('astrophysical/VE', data=VE+' Km/s')
    out_f.create_dataset('astrophysical/VESC', data=VESC+' Km/s')
    out_f.create_dataset('astrophysical/RHO_DM', data=RHO_DM+' GeV/cm3')
    ...
    for i in range(len(all_total_rate_list)):
        for j in range(len(all_total_rate_list[i])):
            ...
            threshold_index = int(all_total_rate_list[i][j][0][1])
            str_time_index = str(int(all_total_rate_list[i][j][0][2]))

            mass = dm_properties_dict['mass_list'][mass_index]
            threshold = physics_parameters['threshold'][threshold_index]
            ...
            if threshold_index not in total_rate_dict[str_time_index][mass_index]:
                total_rate_dict[str_time_index][mass_index][threshold_index] = 0
            ...
            if threshold_index not in total_diff_rate_dict[str_time_index][mass_index]:
                total_diff_rate_dict[str_time_index][mass_index][threshold_index] = \
            ...
            if threshold_index not in total_binned_rate_dict[str_time_index][mass_index]:
                total_binned_rate_dict[str_time_index][mass_index][threshold_index] = \
                        np.zeros(len(all_binned_rate_list[i][j][1]))
            
            total_rate_dict[str_time_index][mass_index][threshold_index] += \
