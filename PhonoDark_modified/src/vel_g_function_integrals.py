...
def integrandSHM(v, V0):
    
    return v*np.exp(-v**2/V0**2)

def KSHM(v, V0):
    return v**2*np.exp(-v**2/V0**2)

def g0_func_SHM(q_vec, omega, m, vE_vec, v_minus):
    
    q_mag = np.linalg.norm(q_vec)
    
    res0, err0 = quad(KSHM, 0, const.VESC, args=(const.V0))
    k = 4*const.PI*res0
    
    c1 = (2*const.PI)**2/(q_mag*k)
   
    v_minus = v_minus_func(q_vec, omega, m, vE_vec)

    if v_minus <= const.VESC:

        res, err = quad(integrandSHM, v_minus, const.VESC, args=(const.V0))
        return c1*res
...
# def integrandTSA(v, q, V0):
#     return v*(1-(1-q)*v**2/V0**2)**(1/(1-q))

# def KTSA(v, q, V0):
#     return v**2*(1-(1-q)*v**2/V0**2)**(1/(1-q))

# q=1-(const.V0/const.VESC)**2

# def g0_func_TSA(q_vec, omega, m, vE_vec, v_minus):
    
#     q_mag = np.linalg.norm(q_vec)
    
#     res0, err0 = quad(KTSA, 0, const.VESC, args=(q, const.V0))
#     k = 4*const.PI*res0
...
#         res, err = quad(integrandTSA, v_minus, const.VESC, args=(q, const.V0))
#         return c1*res
...
def integrandTSA(v, VESC, V0):
    return v*(1-v**2/VESC**2)**(VESC**2/V0**2)

def KTSA(v, VESC, V0):
    return v**2*(1-v**2/VESC**2)**(VESC**2/V0**2)
...
def g0_func_TSA(q_vec, omega, m, vE_vec, v_minus):
    ...
    res0, err0 = quad(KTSA, 0, const.VESC, args=(const.VESC, const.V0))
    k = 4*const.PI*res0
    ...
    if v_minus <= const.VESC:
        ...
        res, err = quad(integrandTSA, v_minus, const.VESC, args=(const.VESC, const.V0))
        return c1*res
...
def integrandEMP(v, p, V0, VESC):
    return v*np.exp(-v/V0)*(VESC**2-v**2)**p

def KEMP(v, p, V0, VESC):
    return v**2*np.exp(-v/V0)*(VESC**2-v**2)**p

def g0_func_EMP(q_vec, omega, m, vE_vec, v_minus):
    
    q_mag = np.linalg.norm(q_vec)
    
    res0, err0 = quad(KEMP, 0, const.VESC, args=(p,const.V0, const.VESC))
    k = 4*const.PI*res0
    ...
    if v_minus <= const.VESC:
        ...
        res, err = quad(integrandEMP, v_minus, const.VESC, args=(p,const.V0, const.VESC))
        return c1*res
...
# def integrandEMP(v, p, VESC):
#     return v*(VESC**2-v**2)**p

# def KEMP(v, p, VESC):
#     return v**2*(VESC**2-v**2)**p

# def g0_func_EMP(q_vec, omega, m, vE_vec, v_minus):
    
#     q_mag = np.linalg.norm(q_vec)
    
#     res0, err0 = quad(KEMP, 0, const.VESC, args=(p, const.VESC))
#     k = 4*const.PI*res0
...
#         res, err = quad(integrandEMP, v_minus, const.VESC, args=(p, const.VESC))
#         return c1*res
